...
const mockFetchCategories = vi.spyOn(CategoriesApi, 'fetchAllCategories')
...
const mockedCategoriesData = {
  ...
  totalTime: '0.013',
  partial: false,
  canonicalUrl: '/v1/categories?show=name,id&format=json&apiKey=VEu4DRF1Wwgl54oI4TerpOTq',
  categories: [
    {
      name: 'Gift Ideas',
      id: 'abcat0010000'
...
describe('Categories API Service', () => {
  ...
  it('should fetch all categories', async () => {
    mockFetchCategories.mockResolvedValue(mockedCategoriesData)

    const response = await CategoriesApi.fetchAllCategories()

    expect(mockFetchCategories).toHaveBeenCalled()
    expect(response).toEqual(mockedCategoriesData)
  })
})
